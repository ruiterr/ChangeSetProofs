...
Module Type FreeGroupsSig.
Parameter (alphabet : Type).
Axiom (alphabet_eq_dec: forall a b:alphabet,{a=b} + {a<>b}).

Parameter (opposite: alphabet->alphabet).
Axiom (opposite_involution: forall a:alphabet, opposite (opposite a) = a).
End FreeGroupsSig.

Module FreeGroups (a: FreeGroupsSig).
Definition alphabet := a.alphabet.
Definition alphabet_eq_dec := a.alphabet_eq_dec.
Definition opposite := a.opposite.
Definition opposite_involution := a.opposite_involution.
...
Module MyFirstGroup <: FreeGroupsSig.

  Inductive testAlphabet := 
    | Op
    | Op_inv.
  Definition alphabet := testAlphabet.

  Definition opposite (x:alphabet) := match x with
    | Op => Op_inv
    | Op_inv => Op
  end.
