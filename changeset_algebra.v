...
Lemma tailIsReduced: ∀ (op: ops.Operation) (l: opList), reduced(op::l) → reduced(l).
  intros.
  unfold reduced.
...
Lemma tailIsReduced2: ∀ (l l': opList) (op:ops.Operation), op::l'=l → reduced(l) → reduced(l').
intros.
  ...
  + apply reducedImpliesFirstTwoOpeartionsNotInverses with (A:=a :: o :: opsA) (T:=opsA) (x:=a) (y:=o) in H; auto.
    ...
    assert(∀A t x y, reduced(A) → (rev A) = (y::t) → y ≠ x⁻¹ → reduced(A ++ [x]))%O. {
      ...
      assert(reduced A'_rev). {
        ...
        - contradiction.
        - rewrite H3 in H2.
          rewrite ops.opInvertInvolution in H2.
    ...
    * rewrite ops.opInvertInvolution.
      auto.
Qed.
...
- revert H.
  ...
  + intros.
    ...
    {
      rewrite_nat.
      destruct (list_beq ops.Operation Op_eqb (OperationGroup.reduction (A ++ B)) []) eqn:H_squashEmpty.
      ...
      - assert ((OperationGroup.reduction (A ++ B)) ≠ []). {
        ...
        assert(∀A B, (reduced A) → (reduced B) → (length A) = (length B) → (OperationGroup.reduction (A ++ B)) ≠ [] → ∃a b ta sb, (A=(a::ta)) ∧ (B=sb ++ [b]) ∧ (OperationGroup.reduction (A ++ B)) = a::(OperationGroup.reduction (ta ++ sb))++[b]). {
          ...
          - intros.
            ...
            destruct (list_beq ops.Operation Op_eqb (OperationGroup.reduction (A0 ++ (rev sb))) []) eqn:H_innerEmpty. {
        ...
        2: {
          ...
          rewrite revSingle.
          rewrite <-cons_to_app.
          assert (∀ x y : ops.Operation, ((x = y) → (Op_eqb x y) = true)) as Op_eq_eqb. {
            intros.
            rewrite H5.
            apply Op_eqb_refl.
          }
          destruct (list_eq_dec ops.Operation Op_eqb Op_eqb_eq Op_eq_eqb (OperationGroup.reduction ((a :: t) ++ sb)) []) eqn:H_emptyInner.
          - rewrite e.
            simpl.
            apply OperationGroup.single_letter_reduced.
